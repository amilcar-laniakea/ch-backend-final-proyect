...
class ProductManager {
  ...
  async addProduct({
    ...
    return { data: product, message: `Product with ID ${id}: added successfully` };

  }

  async deleteProduct(id) {
    if (isNaN(id)) {
      return { data: null, message: "product id is required and must be a number" };
    }
  ...
  async getProductById(id) {
    ...
    if (!product) {
      return { data: null, message: `Product with requested ID ${id}: not found` };
    }
  ...
  async updateProduct(
    ...
    if (productIndex === -1) { return { data: null, message: `Product with requested ID ${id}: not found`}}
    ...
    error = error + validateTypeProductData("description", "string", description);
    ...
    return { data: this.products[productIndex], message: `Product with requested ID ${id}: updated successfully.`};
  }
  ...
  async getProducts(limit) {
    ...
    if (!isNaN(limitParam) && limitParam > 0) {
     ...
     return { data: limitProducts, message: limitProducts.length === 0 ? "No products available" : null};
  }

  async checkAvailableProductStock(id, quantity) {
    if (isNaN(id)) return { status: false, message: "product id must be a number" };
    if (isNaN(quantity)) return { status: false, message: "quantity must be a number" };
    this.products = await this.#manageFile();
    const product = this.products.find((p) => p.id === id);
    if (!product) return { status: false, message: `Product with requested ID ${id}: not found` };
    if (product.stock < quantity) return { status: false, message: `Product with requested ID ${id}: out of stock` };
